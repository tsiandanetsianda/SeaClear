...
export const TextInput = ({ type, className, hasDiv = true, inputType = "text" }) => {
  return (
    <div className={`text-input ${type} ${className}`}>
      {type === "default" && (
        <>
          <>{hasDiv && <div className="text-wrapper-6">Placeholder</div>}</>
        </>
      )}
      ...
      {["right-icon", "two-icons"].includes(type) && (
        <>
          ...
          <ButtonIconHelpCircle1 className="instance-node-2" />
        </>
      )}
      ...
      {["input-with-dropdown", "website-input"].includes(type) && (
        <>
          ...
          <input
            ...
            placeholder={
              type === "input-with-dropdown" ? "Placeholder" : type === "website-input" ? "www.relume.io" : undefined
            }
            type={inputType}
          />
        </>
